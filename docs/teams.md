...
> **Direct `postgres://` team-cloud connections are deprecated (v2.2)** — they connect straight to the database, so row-level security does not apply to them **at all**: a grandfathered direct connection reads and writes every row in every shared table, including rows marked `private`. New direct connections are rejected with guidance to a hosted Teams server; existing ones keep working **silently** (as of v2.2.2 there is no GUI warning or connect-time log — the deprecation is documented here as a breaking change rather than surfaced at runtime). Until you migrate, network-isolate the cloud Postgres so only the hosted server (and the people you'd trust with every byte in it) can reach it. (Using Postgres as a Lattice's own storage backend is unrelated and unaffected.)
