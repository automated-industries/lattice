...
> **Direct `postgres://` team-cloud connections are deprecated (v2.2)** — they bypass the per-recipient sync filter and so can't enforce row-level security. New direct connections are rejected with guidance to a hosted Teams server; existing ones keep working but warn on connect. (Using Postgres as a Lattice's own storage backend is unrelated and unaffected.)
