...
Neither enforces the **parallel file system structure** that example-app uses today — the pattern where each entity type gets a directory, a global index file, per-entity subdirectories, and per-entity files for each relationship type.
...
## Reference Implementation (example-app)
...
✗ Removed orphaned file: agents/agent-b/RULES.md (relationship removed from definition)
...
DRIFT DETECTED — run `lattice reconcile` to fix:
  ...
  ORPHAN   agents/agent-b/FILES.md (file — relationship removed)
  STALE    agents/agent-a/AGENT.md (content hash mismatch)
...
it('removes entity directory when entity is deleted from DB', async () => {
  ...
  expect(existsSync(join(outputDir, 'agents/agent-a'))).toBe(true);
});
...
it('leaves entity directory in place if protected files exist', async () => {
  ...
  writeFileSync(join(outputDir, 'agents/agent-b/SESSION.md'), 'session data');
  ...
  expect(existsSync(join(outputDir, 'agents/agent-b/AGENT.md'))).toBe(false);
...
it('removes old directory and creates new one when slug changes', async () => {
  ...
  // Change agent slug from 'agent-b' to 'agent-b-v2'
  await db.update('agents', 'agent-b-id', { slug: 'agent-b-v2' });
  ...
  expect(existsSync(join(outputDir, 'agents/agent-b-v2/AGENT.md'))).toBe(true);
});
```
...
it('creates new relationship files when relationship added to definition', async () => {
  ...
  expect(existsSync(join(outputDir, 'agents/agent-b/SKILLS.md'))).toBe(true);
});
```
...
it('removes stale relationship file when removed from definition', async () => {
  ...
  expect(existsSync(join(outputDir, 'agents/agent-b/AGENT.md'))).toBe(true);
});
```
...
it('removes stale manyToMany file after all related entities are deleted', async () => {
  ...
  // agent-b has SKILLS.md because it has skills
  expect(existsSync(join(outputDir, 'agents/agent-b/SKILLS.md'))).toBe(true);

  // Remove all of agent-b's skills from the junction table
  await db.delete('agent_skills', /* all agent-b entries */ ...);
...
it('manifest reflects what was written each cycle', async () => {
  ...
  expect(manifest.entityContexts['agents'].entities['agent-b']).toContain('SKILLS.md');
  expect(manifest.entityContexts['agents'].entities['agent-b']).toContain('AGENT.md');

  // Remove agent-b's skills → SKILLS.md omitted next cycle
  await removeAllAgentSkills(db, 'agent-b-id');
  await db.render(outputDir);

  const manifest2 = readManifest(outputDir);
  // Manifest updated: SKILLS.md no longer listed for agent-b
  expect(manifest2.entityContexts['agents'].entities['agent-b']).not.toContain('SKILLS.md');
