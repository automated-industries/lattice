...
**Since 5.5 this holds for EVERY table, not only masked ones.** A member is never
granted table-level `SELECT` on a base table; every member-readable table gets a
`<table>_v` view, masking where an audience is declared and a plain pass-through
where none is. Members additionally hold column-level `SELECT` on the columns the
view does not mask, because a write has to name the row it changes and an upsert
has to name the columns it sets — those columns are row-scoped by the same RLS
predicate either way.

The reason for the change is worth stating, because it is the whole design: the
previous model granted direct read whenever a table had **no recorded secret
columns**, and that is indistinguishable from a table whose column policy was lost,
restored without, or stranded by a rename. A bookkeeping slip therefore became a
data exposure. With no unmasked branch, the worst a lost policy can now produce is
a stale view — visible and repairable — instead of cleartext. For the same reason a
view is never rebuilt looser than the one already standing: the view's own
definition is treated as the record of what was masked, since Postgres stores it
and restoring the policy table cannot erase it. Only an explicit change to a
column's audience may relax masking.
