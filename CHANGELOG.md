...
- **Columns and links are edited separately.** The entity editor splits a table's fields into **Columns** (scalar data) and **Links** (foreign-key relationships) — they're different things and edit differently:
  - **Columns:** system columns (`id` / `created_at` / `updated_at` / `deleted_at`) render read-only (name + type fixed). Editable scalar columns expose an inline name + a `secret` flag, staged behind **one "Save changes"** button (no per-row save). **Add column** offers only the scalar types `text` / `integer` / `real` / `boolean` — `uuid` is reserved for keys and not user-selectable.
  - **Links:** read-only (`name → target`). A link is created via **"Add link"** (pick the target entity; the FK column is named `<target>_id`) and can't be edited once created — only **destroyed** (owner-only). Junction-table legs show read-only with no destroy; remove the whole relationship via **Delete relationship**.
  - **Backend-enforced.** All of the above is enforced by the API, not just the UI, so a bad data model can't be created by hand: `POST …/columns` rejects system names, non-scalar types, and any `ref`; `…/columns/:c/rename` rejects system + FK columns; the new `POST …/entities/:t/links` + `DELETE …/entities/:t/links/:col` are owner-gated; the secret-toggle route rejects system + link columns. Column types are surfaced on `/api/entities` to drive the editor.
