...
---
Deployed piut v2.4.1 to production. Build succeeded in 47s.
===`;
  ...
  it('parses a single valid entry', () => {
    ...
    expect(entry.id).toBe('2026-03-12T15:30:42Z-cortex-a1b2c3');
    expect(entry.type).toBe('event');
    expect(entry.timestamp).toBe('2026-03-12T15:30:42Z');
    expect(entry.project).toBe('piut');
    expect(entry.tags).toEqual(['deploy', 'production']);
    expect(entry.body).toContain('Deployed piut v2.4.1');
  });
...
`;
    const result = parseMarkdownEntries(content, 'cortex');
    expect(result.errors).toHaveLength(0);
    expect(result.entries).toHaveLength(2);
  ...
  it('returns empty for content with no headings', () => {
    const result = parseMarkdownEntries('no headings here', 'cortex');
...
describe('generateEntryId', () => {
  it('generates deterministic id', () => {
    const id1 = generateEntryId('2026-03-12T10:00:00Z', 'cortex', 'body text');
    const id2 = generateEntryId('2026-03-12T10:00:00Z', 'cortex', 'body text');
    expect(id1).toBe(id2);
  });

  it('differs for different bodies', () => {
    const id1 = generateEntryId('2026-03-12T10:00:00Z', 'cortex', 'body A');
    const id2 = generateEntryId('2026-03-12T10:00:00Z', 'cortex', 'body B');
    expect(id1).not.toBe(id2);
  });

  it('has format {timestamp}-{agent}-{6-char-hash}', () => {
    const id = generateEntryId('2026-03-12T10:00:00Z', 'cortex', 'body');
    expect(id).toMatch(/^2026-03-12T10:00:00Z-cortex-[a-f0-9]{6}$/);
  });
...
describe('validateEntryId', () => {
  ...
  it('rejects tampered body', () => {
    const id = generateEntryId('2026-03-12T10:00:00Z', 'cortex', 'original body');
    expect(validateEntryId(id, 'modified body')).toBe(false);
  });

  it('rejects id with wrong hash length', () => {
    expect(validateEntryId('2026-03-12T10:00:00Z-cortex-ab', 'body')).toBe(false);
  });
});
