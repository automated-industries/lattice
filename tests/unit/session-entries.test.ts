...
describe('createReadOnlyHeader', () => {
  it('returns generic text with no args', () => {
    ...
    // Default generator + docsRef are the generic public product name (the exact header
    // string is pinned by the READ_ONLY_HEADER equality test below).
    expect(header).toContain('generated by Lattice');
    expect(header).toContain('the Lattice documentation');
    expect(header).toContain('SESSION.md');
  });
