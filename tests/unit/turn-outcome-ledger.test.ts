...
import { confirmationEvidence } from '../../src/gui/ai/chat.js';
...
describe('turn outcome ledger', () => {
  ...
  const ledgerFor = (text = '', declined = false): TurnOutcomeLedger =>
    new TurnOutcomeLedger({ evidence: { text, declined } });
  ...
  it('lets the same plan through once the user has been asked about every target', async () => {
    const asked = 'Remove Contacts (3 records) and Deals (2 records)? Yes, remove both';
    const ledger = ledgerFor(asked);
    await executeFunction(
      ctx,
      'delete_entity',
      { name: 'contacts', resolution: 'delete_data' },
  ...
  it('keeps the gate closed when the confirmation only named some of the targets', async () => {
    const ledger = ledgerFor('Remove Contacts (3 records)? Yes');
    await executeFunction(
      ctx,
      'delete_entity',
      { name: 'contacts', resolution: 'delete_data' },
  ...
  it('keeps the gate closed when the user just said no', async () => {
    const ledger = ledgerFor('Remove Contacts and Deals? No, keep them', true);
    await executeFunction(
      ctx,
      'delete_entity',
      { name: 'contacts', resolution: 'delete_data' },
  ...
  describe('confirmation evidence', () => {
    it('counts the last question the user was actually shown', () => {
      ...
      expect(namedIn(evidence.text, 'contacts')).toBe(true);
      expect(namedIn(evidence.text, 'deals')).toBe(true);
    ...
    it('never counts a server-authored context note as the user agreeing', () => {
      ...
      expect(namedIn(evidence.text, 'contacts')).toBe(false);
    });
