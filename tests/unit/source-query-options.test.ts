...
function createTestDb() {
  ...
  // Seed data
  db.insert('team', { id: 't1', name: 'Alpha' });
  db.insert('agent', { id: 'a1', name: 'Alice', team_id: 't1', status: 'active' });
  db.insert('agent', { id: 'a2', name: 'Bob', team_id: 't1', status: 'active' });
  db.insert('agent', { id: 'a3', name: 'Charlie', team_id: 't1', status: 'inactive' });
  db.insert('agent', { id: 'a4', name: 'Deleted', team_id: 't1', status: 'active' });
  db.update('agent', 'a4', { deleted_at: '2026-01-01T00:00:00Z' });
  db.insert('skill', { id: 's1', name: 'TypeScript' });
  db.insert('skill', { id: 's2', name: 'Python' });
  db.insert('skill', { id: 's3', name: 'Deleted Skill' });
  db.update('skill', 's3', { deleted_at: '2026-01-01T00:00:00Z' });
  db.insert('agent_skill', { agent_id: 'a1', skill_id: 's1' });
  db.insert('agent_skill', { agent_id: 'a1', skill_id: 's2' });
  db.insert('agent_skill', { agent_id: 'a1', skill_id: 's3' });
...
describe('hasMany with query options', () => {
  ...
  it('orderDir: desc reverses sort', () => {
    const rows = resolveEntitySource(
      { type: 'hasMany', table: 'agent', foreignKey: 'team_id', softDelete: true, orderBy: 'name', orderDir: 'desc' },
      teamRow, 'id', adapter,
    );
  ...
  it('limit caps result count', () => {
    const rows = resolveEntitySource(
      { type: 'hasMany', table: 'agent', foreignKey: 'team_id', softDelete: true, orderBy: 'name', limit: 2 },
      teamRow, 'id', adapter,
    );
...
describe('manyToMany with query options', () => {
  ...
  it('basic manyToMany without options (backward compat)', () => {
    const rows = resolveEntitySource(
      { type: 'manyToMany', junctionTable: 'agent_skill', localKey: 'agent_id',
        remoteKey: 'skill_id', remoteTable: 'skill' },
      agentRow, 'id', adapter,
    );
  ...
  it('orderBy sorts remote table results', () => {
    const rows = resolveEntitySource(
      { type: 'manyToMany', junctionTable: 'agent_skill', localKey: 'agent_id',
        remoteKey: 'skill_id', remoteTable: 'skill', softDelete: true, orderBy: 'name' },
      agentRow, 'id', adapter,
    );
  ...
  it('limit caps manyToMany results', () => {
    const rows = resolveEntitySource(
      { type: 'manyToMany', junctionTable: 'agent_skill', localKey: 'agent_id',
        remoteKey: 'skill_id', remoteTable: 'skill', softDelete: true, orderBy: 'name', limit: 1 },
      agentRow, 'id', adapter,
    );
    expect(rows).toHaveLength(1);
...
describe('belongsTo with query options', () => {
  ...
  beforeEach(() => {
    ...
    db.update('team', 't1', { deleted_at: '2026-01-01T00:00:00Z' });
  });
...
describe('sourceDefaults merging', () => {
  it('merges sourceDefaults into hasMany sources', () => {
  ...
  it('per-file source options override sourceDefaults', () => {
    const db = createTestDb();
    const rendered: Row[][] = [];
    ...
    db.defineEntityContext('team', {
      ...
      files: {
        'AGENTS.md': {
          source: { type: 'hasMany', table: 'agent', foreignKey: 'team_id',
                    orderBy: 'name', orderDir: 'desc' }, // override direction
          render: (rows) => {
            rendered.push(rows);
            return rows.map(r => r.name).join(', ');
          },
  ...
  it('does not affect custom or self sources', () => {
    const db = createTestDb();
    let selfRows: Row[] = [];
    let customRows: Row[] = [];
    ...
    db.defineEntityContext('team', {
      ...
      files: {
        'TEAM.md': {
          source: { type: 'self' },
          render: (rows) => { selfRows = rows; return 'self'; },
        },
        'CUSTOM.md': {
          source: { type: 'custom', query: (row, adapter) =>
            adapter.all('SELECT * FROM agent WHERE team_id = ?', [row.id]) },
          render: (rows) => { customRows = rows; return 'custom'; },
        },
      },
    });
