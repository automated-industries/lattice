...
describe('parseSessionWrites — valid update entry', () => {
  it('parses op/table/target/fields correctly', () => {
    ...
    expect(entry.target).toBe('cortex');
...
describe('parseSessionWrites — multiple write entries', () => {
  it('parses all entries correctly', () => {
    const entry1 = makeEntry({ target: 'cortex' }, 'status: active');
    ...
    expect(result.entries[0]!.target).toBe('cortex');
...
describe('generateWriteEntryId — deterministic', () => {
  it('produces the same ID for the same inputs', () => {
    const id1 = generateWriteEntryId('2026-03-25T10:30:00Z', 'cortex', 'update', 'agent', 'hal');
    const id2 = generateWriteEntryId('2026-03-25T10:30:00Z', 'cortex', 'update', 'agent', 'hal');
    expect(id1).toBe(id2);
  });
});
...
describe('generateWriteEntryId — different inputs produce different IDs', () => {
  it('produces different IDs when inputs differ', () => {
    const id1 = generateWriteEntryId('2026-03-25T10:30:00Z', 'cortex', 'update', 'agent', 'hal');
    const id2 = generateWriteEntryId('2026-03-25T10:30:00Z', 'cortex', 'create', 'agent', 'hal');
    expect(id1).not.toBe(id2);

    const id3 = generateWriteEntryId('2026-03-25T10:30:00Z', 'cortex', 'update', 'agent', 'cortex');
