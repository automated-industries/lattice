...
`;
    ...
    expect(slugFn({ slug: 'forge', name: 'Forge' })).toBe('forge');
  });
...
`;
    ...
    const output = renderFn([{ name: 'Forge', slug: 'forge' }]);
    expect(output).toContain('- ');
    expect(output).toContain('name: Forge');
  });
...
`;
    ...
    expect(output).toContain('| Forge | forge |');
  });
...
`;
    const result = parseConfigString(yaml, configDir);
    const renderFn = result.entityContexts[0]!.definition.files['AGENT.md']!.render;
    const output = renderFn([{ name: 'Forge' }]);
    expect(JSON.parse(output)).toEqual([{ name: 'Forge' }]);
  });
