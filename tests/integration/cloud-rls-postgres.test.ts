...
function memberPool(schema: string, role: string): pg.Pool {
...
afterEach(async () => {
  ...
  for (const r of roles.splice(0)) {
    await admin.query(`DROP OWNED BY "${r}"`).catch(() => {});
    await admin.query(`DROP ROLE IF EXISTS "${r}"`).catch(() => {});
  }
  await admin.end();
});

describe.skipIf(!PG_URL)('cloud RLS — direct scoped-role isolation (no server, no app filter)', () => {
  it('a member cannot read, mutate, or re-share another member\'s rows', async () => {
    ...
    // 2) Provision two scoped, non-superuser member roles with table DML only —
    //    no access to the bookkeeping tables.
    await admin.query(`CREATE ROLE "${alice}" LOGIN NOSUPERUSER PASSWORD 'pw'`);
    await admin.query(`CREATE ROLE "${bob}" LOGIN NOSUPERUSER PASSWORD 'pw'`);
    await admin.query(`GRANT USAGE ON SCHEMA "${schema}" TO "${alice}", "${bob}"`);
    await admin.query(
      `GRANT SELECT, INSERT, UPDATE, DELETE ON "${schema}".notes, "${schema}".memo TO "${alice}", "${bob}"`,
    );

    const A = memberPool(schema, alice);
    const B = memberPool(schema, bob);

    // 3) Alice writes and shares some rows via the owner-only helpers.
    await A.query(`INSERT INTO notes (id, body) VALUES ('n1','priv'),('n2','shared-all'),('n3','shared-bob')`);
    ...
    const bobNotes = (await B.query<{ id: string }>(`SELECT id FROM notes ORDER BY id`)).rows.map((r) => r.id);
    expect(bobNotes).toEqual(['n2', 'n3']);
    const bobMemo = (await B.query<{ a: string; b: string }>(`SELECT a,b FROM memo ORDER BY a,b`)).rows;
    expect(bobMemo).toEqual([{ a: 'x', b: '2' }]);

    // Alice sees all of her own.
    const aliceNotes = (await A.query<{ id: string }>(`SELECT id FROM notes ORDER BY id`)).rows.map((r) => r.id);
    ...
    await expect(B.query(`SELECT lattice_set_row_visibility('notes','n1','everyone')`)).rejects.toThrow(
      /only the row owner/i,
    );

    // 7) Bob cannot read the bookkeeping directly, nor disable RLS.
    await expect(B.query(`SELECT count(*) FROM "__lattice_owners"`)).rejects.toThrow(/permission denied/i);
    await expect(B.query(`ALTER TABLE notes DISABLE ROW LEVEL SECURITY`)).rejects.toThrow(/must be owner/i);
