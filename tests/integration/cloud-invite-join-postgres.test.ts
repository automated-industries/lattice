...
describe.skipIf(!PG_URL)('#5.2 invite → join → connect (e2e)', () => {
  it('a redeemed invite creates a NEW active cloud workspace; the local one is untouched', async () => {
    // ── Owner: a fresh database, secured as a cloud with a shared table + rows.
    ...
    // ── Owner GUI: invite an email, capture the token.
    ...
    const invite = (await fetch(`${ownerGui.url}/api/cloud/invite`, {
      method: 'POST',
    ...
    // ── Member: a SEPARATE root with a PRE-EXISTING local workspace.
    ...
    expect(before.current).toBe(localWs.id);

    // ── Member redeems the invite.
    const redeem = (await fetch(`${memberGui.url}/api/cloud/redeem-invite`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ email: 'member@example.com', token: invite.token }),
    }).then((r) => r.json())) as { ok?: boolean; isCloud?: boolean; error?: string };
    expect(redeem.error).toBeUndefined();
    expect(redeem.ok).toBe(true);
    expect(redeem.isCloud).toBe(true);
    ...
    const local = after.workspaces.find((w) => w.id === localWs.id);
    const cloud = after.workspaces.find((w) => w.id !== localWs.id);
