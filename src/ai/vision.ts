...
/**
 * Image extraction (vision): normalize an image with `sharp` and ask the model
 * to describe it. AI-gated — requires a {@link ClaudeAuth}. The model call is
 * injectable via {@link VisionOptions.sender} so it is testable without the SDK
 * or a key. `sharp` and `@anthropic-ai/sdk` are loaded lazily, so importing
 * this module costs nothing until {@link describeImage} runs.
 */
