...
export const computedBuilderCss = `    /* ── Computed-table builder (#/computed/*) ─────────────── */
    ...
    .cb-name-input.cb-invalid { border-color: var(--danger); box-shadow: 0 0 0 2px rgba(239, 68, 68, 0.15); }
    .cb-status { margin-left: auto; font-size: 12.5px; color: var(--text-muted); white-space: nowrap; }
    .cb-error {
      margin: 10px 0; padding: 10px 12px; border: 1px solid rgba(239, 68, 68, 0.4);
      border-radius: 8px; background: rgba(239, 68, 68, 0.06); color: var(--danger-deep); font-size: 13px;
    }
    .cb-card {
    ...
    .cb-field { border: 1px solid var(--border); border-radius: 8px; padding: 10px 12px; margin-bottom: 10px; background: var(--surface); }
    .cb-field-main { display: flex; align-items: center; gap: 8px; }
    .cb-field-name { flex: 1 1 auto; min-width: 0; font-family: 'JetBrains Mono', ui-monospace, SFMono-Regular, Menlo, monospace; padding: 5px 8px; }
    .cb-field-kind { flex: 0 0 auto; padding: 5px 8px; }
    .cb-field-del {
    ...
    .cb-expr { font-family: 'JetBrains Mono', ui-monospace, SFMono-Regular, Menlo, monospace; }
    .cb-chips { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; }
    .cb-chip {
      display: inline-flex; align-items: center; gap: 5px; padding: 2px 6px 2px 8px;
      border: 1px solid var(--border); border-radius: 999px; background: var(--surface-2); font-size: 12.5px;
    }
    .cb-chip-n {
      display: inline-flex; align-items: center; justify-content: center;
      min-width: 15px; height: 15px; border-radius: 999px; background: var(--accent-soft);
    ...
    .cb-chip-input { border: 0 !important; background: none !important; min-width: 160px; padding: 4px 2px; font-size: 12.5px; }
    .cb-chip-input:focus { outline: none; }
    .cb-actions { display: flex; align-items: center; gap: 8px; margin: 14px 0; }
    .cb-refresh-log {
      margin: 10px 0; padding: 10px 12px; border: 1px solid var(--border); border-radius: 8px;
    ...
    .cb-preview-wrap { overflow-x: auto; border: 1px solid var(--border); border-radius: 8px; }
    .cb-preview-table { margin: 0; }
    .cb-sql { margin: 12px 0; }
    .cb-sql summary { cursor: pointer; font-size: 12.5px; color: var(--text-muted); }
    .cb-sql pre {
      margin: 8px 0 0; padding: 10px 12px; border: 1px solid var(--border); border-radius: 8px;
    ...
    .fs-computed-badge {
      ...
      font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; white-space: nowrap;
    }
    .fs-computed-note { margin: 4px 0 10px; font-size: 12.5px; color: var(--text-muted); }
    .fs-computed-fields { display: grid; grid-template-columns: minmax(120px, 200px) 1fr; gap: 6px 14px; padding: 14px; margin: 0; }
    .fs-computed-fields dt { color: var(--text-muted); font-size: 12.5px; padding-top: 1px; }
    .fs-computed-fields dd { margin: 0; min-width: 0; overflow-wrap: anywhere; }

    /* Tables explorer: computed-tier additions. */
    .mt-tier-new {
      margin-left: auto; padding: 1px 8px; font: inherit; font-size: 11.5px; font-weight: 600;
    ...
    .mt-edge-computes { stroke: #0ea5e9; opacity: 0.55; stroke-dasharray: 5 4; }
    ...
    .mt-computed-sqlpre {
      margin: 6px 0 0; padding: 8px 10px; border: 1px solid var(--border); border-radius: 6px;
      background: var(--surface-2); font-size: 11.5px; overflow: auto; white-space: pre-wrap; max-height: 220px;
    }
`;
