...
export const appJs = `
  (function () {
    ...
    // Connect-a-dashboard panel (Dashboard tab + the "Connect dashboard" header
    // button). Walks the user through pointing Lattice at their own dashboard on
    // disk (file or folder), including a copyable prompt that asks Claude to find
    // the exact path. Connecting POSTs the path to /api/connect/dashboard, which
    // serves it in place at / (the built-in view moves to /lattice).
    function renderConnectDashboard(body) {
      // The Claude prompt is built from what the user types about their dashboard
      // (name / description), injected into a template — like the firm-portal
      // request preview. currentPrompt() reads the live description box.
      function promptFor(desc) {
        ...
        return [
          ...
          '',
          'Please reply with:',
          '1) The absolute path to the dashboard. If it is a folder, give the folder that directly contains index.html (for example C:/Users/me/projects/my-dashboard on Windows, or /Users/me/projects/my-dashboard on Mac). If it is a single file, give the full path to that .html file.',
          '2) Whether it is a folder or a single file.',
          '',
          'If you are not sure where it is, search my common locations — Desktop, Documents, Downloads, and any project folders you have worked in — for index.html or other .html files, and list each candidate with its full absolute path so I can pick the right one.',
          '',
      ...
      body.innerHTML =
        ...
        '<div class="cd-step">' +
          ...
          '<p>Describe your dashboard or type its name, then copy the prompt and paste it into Claude (or Claude Code) to find the exact path on your computer.</p>' +
          '<textarea class="cd-desc" id="cd-desc" placeholder="e.g. my fund track record dashboard, or the React site I built last week"></textarea>' +
      ...
      // Live preview: the prompt textarea reflects the description as it is typed.
      ...
      if (connectBtn) connectBtn.addEventListener('click', function () {
        ...
        withBusy(connectBtn, function () {
          return fetchJson('/api/connect/dashboard', {
            ...
            // Land on the dashboard immediately, in THIS window — no separate
            // navigation. The root path now serves the connected dashboard; a
            // hard reload swaps the built-in shell for it (still at /lattice).
            showStatus('ok',
              'Connected ' + (d.mode === 'dir' ? 'folder' : 'file') + ' — opening your dashboard…');
      ...
      // Show the currently connected dashboard (if any) + prefill the field.
      fetchJson('/api/connect/dashboard').then(function (d) {
        if (d && d.path) {
          showStatus('ok',
            'Currently connected: <code>' + escapeHtml(d.path) + '</code> (' + (d.mode === 'dir' ? 'folder' : 'file') + ') —' + liveLinks);
          var input = document.getElementById('cd-path');
    ...
    function refreshConnectButton() {
      fetchJson('/api/connect/dashboard')
        .then(function (d) { setDashboardConnected(d && d.path); })
    ...
    function wireSettingsDrawer() {
      ...
      var connectDashBtn = document.getElementById('connect-dash-btn');
