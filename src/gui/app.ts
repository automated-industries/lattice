...
<body>
  ...
  <header class="topbar">
    ...
    <!-- Account: the connected Claude account + a Disconnect action. Connect
         itself happens at the first-run wall, never here. Hidden until connected
         and in a managed deployment (the operator owns the credential). -->
    <div class="account" id="account" hidden>
      <button class="history-btn" id="account-btn" title="Claude account" aria-label="Claude account" aria-haspopup="true" aria-expanded="false">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
      </button>
      <div class="account-menu" id="account-menu" hidden>
        <div class="account-menu-head">Connected with Claude</div>
        <button type="button" class="account-menu-item danger" id="account-disconnect">Disconnect Claude</button>
      </div>
    </div>
