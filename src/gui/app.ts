...
<body>
  ...
  <header class="topbar">
    ...
    <!-- Account menu. Normal install: the connected model account + a Disconnect
         action (connect happens at the first-run wall, never here). Managed/hosted
         deployment: the signed-in identity + a Log out action. The head text and
         which action shows are set by JS (account-menu.ts) from the config. -->
    <div class="account" id="account" hidden>
      ...
      <div class="account-menu" id="account-menu" hidden>
        ...
        <button type="button" class="account-menu-item" id="account-logout" hidden>Log out</button>
      </div>
    </div>
