...
const HTML_SYSTEM = [
  ...
  '    lattice.provenance(table, id)            → resolves to { object, scope, links, note }',
  ...
  '- For data-driven numbers (stats, counts, queries results), when you want to show WHERE they came from, keep the producing table and row id on the element: `<span data-src-table="<table>" data-src-id="<id>">$N</span>`. Wire the click handler: call `await lattice.provenance(table, id)` → render a small inset popover showing the provenance chain (from --relation--> to links) with an "Open source" control that fires `window.lattice.act()` for the source table/id. This surfaces data lineage and lets users drill to source documents without leaving the dashboard.',
  '',
