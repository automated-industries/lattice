...
const HTML_SYSTEM = [
  ...
  '    lattice.provenance(table, id)            → resolves to { object, scope, links, note, sources }',
  ...
  '',
  'Clickable citations and provenance:',
  '- When the spec names source documents (files), PREFER embedding an explicit FILE_MAP: at the top of your page, write `const FILE_MAP = { "Label A": "<files-id-1>", "Label B": "<files-id-2>", … }` mapping the source labels to their actual file ids. Use this map at render time to resolve badges: check if a citation label has an id in FILE_MAP, render it clickable with `onclick="window.lattice.act(\'open-record\',\'files:\' + FILE_MAP[label])"`, and if no id is found, DROP the interactive affordance (remove `cursor:pointer`, `onclick`, and hover styling — render it as plain text so it never looks clickable but fails silently). Never emit uninterpolated `${...}` template placeholders into HTML attributes.',
  '- For data-driven numbers (stats, counts, queries results), when you want to show WHERE they came from, keep the producing table and row id on the element: `<span data-src-table="<table>" data-src-id="<id>">$N</span>`. Wire the click handler: call `await lattice.provenance(table, id)` → render a small inset popover showing the provenance chain (the `links` array, from --relation--> to) and, for each entry in the `sources` array (`{ table, id, label }` — the actual upstream rows, labeled by their own names), an "Open source" control that fires `window.lattice.act(\'open-record\', table + \':\' + id)`. This surfaces data lineage and lets users drill to the exact source documents without leaving the dashboard.',
  '',
