...
// (Capacity, not a workaround — see CHANGELOG; flagged for review per review policy.)
...
export async function* runChat(opts: RunChatOptions): AsyncGenerator<ChatStreamEvent> {
  ...
  try {
    ...
    // unfinished. Surface it loudly (the fail-loudly rule: no silent truncation) instead of
