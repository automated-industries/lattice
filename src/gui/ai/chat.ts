...
const BASE_SYSTEM_PROMPT = [
  ...
  "- PROACTIVELY wire up relationships when you create or update a record that names other things — a meeting's attendees, a message's sender/recipients, a task's owner, an event's participants. Do not leave those names as plain text on the row. For EACH named person/company: search for the matching existing record (search by the name) and reuse it, creating it only if none exists, then link it (create_relationship first if no link exists yet, else link). Follow the convention already in the data: if other records of that kind are linked to people/companies, a new one MUST be too — check an existing example if unsure. This is part of fulfilling the request; the user should never have to ask you to link the obvious related records. When the new record involves the user themselves (a meeting they attend, a task assigned to them), link THEIR own record too — find it by searching for their name.",
...
export function buildSystemPrompt(
  ...
  const who =
    operatorName && operatorName.trim().length > 0
      ? `\n\n# Who you are assisting\nYou are assisting ${operatorName.trim()}. When the user says "me" / "my", they mean ${operatorName.trim()}; never ask the user for their own name. When you create or update a record the user is part of (a meeting they attend, a task assigned to them), link their OWN record to it too — find it by searching for "${operatorName.trim()}".`
      : '';
