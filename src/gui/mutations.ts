 ...
 */
// A file's `extracted_text` (the full document body, often many KB) was stored
// VERBATIM in every audit row's before/after image. Across a folder ingest that
// is O(N) large blobs in `_lattice_gui_audit`, and any full-table re-scan of that
// table (renders, history reads) then does O(N²) work moving them around. The
// audit log is NOT the source of truth for a file's extracted content — it is
// re-derivable from the file — so cap it to a hash + short preview. Undoing a
// change to `extracted_text` specifically restores the preview (acceptable: the
// text is regenerated by re-ingesting the file); every other column round-trips
// unchanged. Kept intentionally narrow (this one derived column), not a blanket
// truncation of arbitrary user text.
export const AUDIT_TEXT_PREVIEW = 400;
export function capAuditImage(image: unknown): unknown {
  if (!image || typeof image !== 'object') return image;
  const rec = image as Record<string, unknown>;
  const et = rec.extracted_text;
  if (typeof et !== 'string' || et.length <= AUDIT_TEXT_PREVIEW) return image;
...
function buildAuditRow(
  ...
  return {
    ...
    row_id: rowId,
    operation: op,
    before_json: before ? JSON.stringify(capAuditImage(before)) : null,
    after_json: after ? JSON.stringify(capAuditImage(after)) : null,
    undone: 0,
