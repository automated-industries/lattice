...
export async function inviteDirect(
  ...
  // Publish the encrypted envelope to the latticesql.com invite relay so
  // the invitee can join with just (token, email) — no Postgres URL to
  // type. Fire-and-forget; a relay outage doesn't block invite issuance
  // (the cloud-side __lattice_invitations row stays the source of truth).
  if (cloudUrl) {
    void publishInviteEnvelope({
      rawToken: raw,
      email: inviteeEmail,
      cloudUrl,
      teamId,
...
// only — no template-string concatenation into SQL (Part C12 of the
// 1.14 plan).
...
/**
 * Direct-Postgres linkRow — records the link locally; for direct-mode
 * clouds there's no separate cloud mirror, so writing the link row to
 * `__lattice_local_links` is the entirety of the operation (the row
 * itself already lives in the cloud Postgres because the operator's
 * local lattice IS the cloud).
 */
